...
class AgxBatch(models.Model):
    ...
    def _compute_qc_count(self):
        """Count QC inspections linked to each batch.

        Kept lightweight on purpose because the field is used by form onchanges
        and stat buttons; computing via search_count avoids loading unrelated
        one2many data and works safely even when the batch is still unsaved.
        """
        QC = self.env["agx.qc.inspection"]
        for rec in self:
            rec.qc_count = QC.search_count([("batch_id", "=", rec.id)]) if rec.id else 0
