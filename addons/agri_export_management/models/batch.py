...
"""Production Batch models — raw material to finished goods.

Models defined here:
  AgxBatch              — The batch record with costing and yield tracking
  AgxBatchInput         — Raw material consumption lines
  AgxBatchOutput        — Finished goods output lines (Grade × Size × Lot)
  AgxBatchScrap         — Waste/loss lines (5 types, 'other' requires reason)
  AgxPackagingMaterial  — Packaging material catalogue
  AgxBatchPackagingLine — Packaging material usage per batch

Workflow: Draft → In Progress → Done (or Cancelled)
Key side effect: Mark Done triggers stock moves and lot creation.
"""
"""
...
class AgxBatch(models.Model):
    ...
    costing_status = fields.Selection(# actual = pulled from PO, manual = user-entered
    ...
    stock_move_state = fields.Selection(# Mirrors the linked stock.picking state
    ...
    def action_view_stock_moves(self):
        """Open the list of stock moves generated when this batch was completed."""
        self.ensure_one()
        return {
            "type": "ir.actions.act_window",
    ...
    def action_view_receipts(self):
        """Open incoming receipts linked via the evaluation PO."""
        self.ensure_one()
        return {
            "type": "ir.actions.act_window",
...
class AgxBatchScrap(models.Model):
    ...
    scrap_type = fields.Selection(# Type determines reporting category in yield analysis
    ...
    @api.onchange("product_id")
    def _onchange_product_id(self):
        """Auto-fill grade/size/UoM from the selected finished-goods variant."""
        if self.product_id:
