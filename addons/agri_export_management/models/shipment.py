...
class AgxShipment(models.Model):
    ...
    container_ids = fields.One2many(
        "agx.shipment.container", "shipment_id", string="Containers", copy=True
    )
    line_ids = fields.One2many(
        "agx.shipment.line", "shipment_id", string="Products", copy=True
    )
    lot_line_ids = fields.One2many(
        "agx.shipment.lot.line", "shipment_id", string="Reserved Lots", copy=True
    )
    ...
    gross_margin_pct = fields.Float(
        compute="_compute_profitability", store=True, digits=(16, 2)
    )
    note = fields.Html()
    ...
    def _compute_profitability(self):
        for rec in self:
            ...
            effective_container_count = (
                rec.container_count or len(rec.container_ids) or 1
            )
            rec.cost_per_container = (
                rec.total_logistics_cost / effective_container_count
                if effective_container_count
                else 0.0
            )
            rec.revenue_amount = (
                rec.sale_order_id.amount_untaxed if rec.sale_order_id else 0.0
            )
            rec.gross_profit_amount = rec.revenue_amount - rec.total_logistics_cost
            # Warn in chatter when margin goes negative but never crash
    ...
    def create(self, vals_list):
        for vals in vals_list:
            if vals.get("name", _("New")) == _("New"):
                vals["name"] = (
                    self.env["ir.sequence"].next_by_code("agx.shipment") or _("New")
                )
        ...
        for rec in self:
                    ...
                    Container.create(
                        {
                            "shipment_id": rec.id,
                            "sequence": (index + 1) * 10,
                            "name": "{}-C{}".format(rec.name, index + 1),
            ...
            if rec.container_ids:
                ...
                if first:
                    ...
                    if header_vals:
                        rec.with_context(agx_skip_container_sync=True).write(
                            header_vals
                        )
    ...
    def _lock_reservation_scope(self, candidate_quant_ids, lot_pairs):
        ...
        self.env["stock.quant"].flush_model(
            ["company_id", "product_id", "lot_id", "quantity", "reserved_quantity", "location_id"]
        )
        self.env["agx.shipment.lot.line"].flush_model(
            ["shipment_id", "product_id", "lot_id", "reserved_qty"]
        )
        self.env.cr.execute(
            "SELECT id FROM agx_shipment WHERE id = %s FOR UPDATE", (self.id,)
        )
        ...
        if lot_pairs:
            ...
            self.env.cr.execute(
                ...
                SELECT ll.id
                  ...
                  JOIN (VALUES {values}) AS scope(product_id, lot_id)
    ...
    def _get_default_stock_location(self):
        self.ensure_one()
        # FIX: add explicit order for deterministic result
    ...
    def _validate_delivery_settings(self):
        ...
        if not source_location or not customer_location or not outgoing_type:
            raise UserError(
                _(
                    "Please configure Finished Goods Location and "
                    "Outgoing Delivery Type in Settings."
                )
            )
        ...
        for rec in self:
            ...
            if picking and picking.state not in ("done", "cancel"):
                ...
                picking.message_post(
                    body=_("Delivery rebuilt from latest AGX reservation.")
                )

    def _build_delivery_picking(self):
        Move = self.env["stock.move"]
        for rec in self:
            source_location, customer_location, outgoing_type = (
            ...
            picking = self.env["stock.picking"].create(
                {
                    ...
                    # FIX: set flow_type directly at creation time
    ...
    def action_reserve(self):
        for rec in self:
            ...
            candidate_quant_ids, candidate_pairs = (
                rec._get_candidate_reservation_scope()
            )
    ...
    def action_ship(self):
        ...
        for rec in self:
            ...
            for move in picking.move_ids.filtered(
                ...
                reserved_lines = rec.lot_line_ids.filtered(
                ...
                move.product_uom_qty = sum(
                    reserved_lines.mapped("reserved_qty")
                )
        ...
        for rec in records:
            ...
            if rec.shipment_id and rec.shipment_id.container_count != len(
                ...
                rec.shipment_id.with_context(
                    ...
                    {"container_count": len(rec.shipment_id.container_ids)}
                )
        return records
...
class AgxShipmentLine(models.Model):
        ...
        for rec in self:
            if (
                ...
                raise ValidationError(
                    _(
                        "The selected container must belong to the same shipment."
                    )
                )
    ...
    def _compute_allocated_costs(self):
        # FIX: pre-aggregate denominators per shipment outside the inner loop
        # to avoid redundant sum() calls on every cost_line × line combination.
        shipment_denominators = {}
        for line in self:
            sid = line.shipment_id.id
            if sid not in shipment_denominators:
                lines = line.shipment_id.line_ids
                shipment_denominators[sid] = {
            ...
            denoms = shipment_denominators[sid]
            total_allocated = 0.0
            for cost_line in line.shipment_id.cost_line_ids:
                basis = cost_line.allocation_basis
                if basis == "carton":
                    denominator = denoms["carton"]
                    numerator = line.carton_qty
                elif basis == "net_weight":
                    denominator = denoms["net_weight"]
                    numerator = line.net_weight
                elif basis == "gross_weight":
                    denominator = denoms["gross_weight"]
                    numerator = line.gross_weight
                elif basis == "equal":
                    denominator = denoms["count"]
                    numerator = 1.0 if line.id else 0.0
                else:
                    denominator = denoms["qty"]
                    numerator = line.product_qty
                if denominator:
                    total_allocated += cost_line.effective_amount * (
                        numerator / denominator
                    )
            line.allocated_logistics_cost = total_allocated
            line.cost_per_carton = (
                total_allocated / line.carton_qty if line.carton_qty else 0.0
            )
            line.cost_per_qty = (
                total_allocated / line.product_qty if line.product_qty else 0.0
            )
    ...
    def _check_reserved_qty(self):
        for rec in self:
            ...
            if rec.reserved_qty > effective_available:
                raise ValidationError(
                    _(
                        "Reserved quantity cannot exceed the currently available "
    ...
    def _compute_source_amount(self):
        for rec in self:
            ...
            rec.effective_amount = (
                source_amount if rec.cost_source != "manual" else rec.manual_amount
            )
