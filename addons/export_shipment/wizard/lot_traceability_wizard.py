...
class ExportLotTraceabilityWizard(models.TransientModel):
    ...
    NOTE:
    - We DO NOT ask the user to select lots.
    - We auto-detect finished lots from:
      Invoice -> (Export Shipments) -> Reservation Picking -> done move lines lots
    """
    ...
    def default_get(self, fields_list):
        ...
        # ✅ IMPORTANT:
        # inv.invoice_origin غالبًا رقم الـ SO (مثال: S00012)
        # وإنت عندك export.shipment.sale_order_id مرتبط بـ sale.order
        # فهنا الأفضل نجيب الـ shipment عن طريق الـ sale_order_id.name == invoice_origin
        shipments = self.env["export.shipment"].search([
            ("sale_order_id.name", "=", inv.invoice_origin),
        ], limit=50)
    ...
    def _get_finished_lots_from_shipments(self, shipments):
        """Return lots shipped on the reservation pickings of shipments (done move lines lots)."""
        lots = self.env["stock.lot"]
        for sh in shipments:
            picking = sh.reserved_picking_id
            if not picking:
                continue
            # هنلمّ أي lot اتشحن/اتحجز على الـ picking
    ...
    def _find_production_for_lot(self, lot, product):
        ...
        Search move lines with this lot linked to a production.
        """
        self.ensure_one()
        MoveLine = self.env["stock.move.line"]
        ml = MoveLine.search([
    ...
    def _trace_lot_chain(self, finished_lot):
        """
        Build a trace dict for a finished lot:
        finished -> production -> raw lots -> vendor receipts (per raw lot)
        """
        self.ensure_one()
        ...
        return {
            ...
            "receipt_date": receipt_dates and min(receipt_dates) or False,
        }
        ...
        for wiz in self:
            ...
            for lot in wiz.lot_ids:
                ...
                lines_vals.append({
                    ...
                    "receipt_date": chain["receipt_date"],
                })
